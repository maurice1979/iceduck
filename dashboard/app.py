"""IceDuck analyst dashboard — reads the gold layer (iceduck_gold) directly
via a fresh DuckDB session, using the same resolve-then-iceberg_scan
mechanism as the rest of this project (ADR-0007), not Athena. See
ADR-0011 for why Streamlit + DuckDB was chosen over alternatives.
"""
...
enc_by_month = (
    enc_by_date.groupby(["year", "month", "month_name"], as_index=False)
    .size()
...
with col_a:
    ...
    st.plotly_chart(px.bar(by_class, x="encounter_class", y="count", title="Encounters by class"), use_container_width=True)

with col_b:
    cost_by_org = (
        encounters.merge(organizations, on="organization_id")
        .groupby("name", as_index=False)["total_claim_cost"]
        .sum()
