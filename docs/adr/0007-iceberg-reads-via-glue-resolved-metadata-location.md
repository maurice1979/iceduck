...
- **Athena interoperability, as currently pitched, does not hold against Floci.** The project's central "same physical Iceberg table, readable from both DuckDB and Athena" claim needs either: (a) a caveat that Athena verification is out of scope until Floci supports Iceberg-format Glue tables, or (b) a workaround — e.g. registering a second, Hive-style Parquet-external Glue table pointing at the same underlying data files specifically for Athena to read (not a real Iceberg read, just format-compatible), which would undercut the "one real Iceberg table, two engines" claim rather than support it. No workaround is implemented as part of this spike; tracked in `docs/TODO.md` for Build Order step 8 to make a real decision once gold tables exist.
